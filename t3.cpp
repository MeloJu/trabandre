...
void RealizaBuscaSimples(Indice ind, string p){
	char LetInicial=p.at(0);
    int pos=LetInicial-'a';
    for(int i=0; i<ind.palavras[pos].size(); i++){
        if (ind.palavras[pos][i].letras == p){
            ImprimeOcorrencias(ind.palavras[pos][i].ocorrencias);
            return;
		}
    }
...
void RealizaBuscaE(Indice ind, string p1, string p2){
	if (palavraexiste(ind, p1) && palavraexiste(ind, p2)){
		char LetInicial=p.at(0);
    	int pos=LetInicial-'a';
		for (int i=0; i<ind.palavras[pos][i].ocorrencias.size(); i++){
			cout<<"arq: "<<ind.palavras[pos][i].ocorrencias.arquivo<<endl;
		}
	} else {
		cout<<"Palavras não encontradas"<<endl;
	}
}

void RealizaBuscaOU(Indice ind, string p1, string p2){
	
}

void MenuBusca(Indice ind){
	int esc;
	string esc2, esc3;
	cout<<"[1] Realizar busca simples"<<endl<<"[2] Realizar busca operador E"<<endl<<"[3] Realizar busca operador OU"<<endl<<"Digite: ";
	cin>>esc;
	switch(esc){
		case 1:
			cout<<endl<<"Palavra desejada: ";
			cin>>esc2;
			RealizaBuscaSimples(ind, esc2);
			system("pause");
            break;
        case 2:
        	cout<<endl<<"Palavras desejadas: ";
			cin>>esc2;
			cin>>esc3;
        	RealizaBuscaE(ind, esc2, esc3);
        	system("pause");
        	break;
        case 3:
        	cout<<endl<<"Palavras desejadas: ";
